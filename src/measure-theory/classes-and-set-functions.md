...
\end{aligned}
$$
于是当 $a \geq b$ 时 $(a, b) = [a, b) = (a, b] = \emptyset$，当 $a > b$ 时 $[a, b] = \emptyset$．$[a, a] = \Set{a}$．
...
\begin{aligned}
  ...
  & \cup \Set{ (a, b] | a < b,\ a, b \in \mathbb{R} } \\
  & \cup \Set{ (-\infty, b] | b \in \mathbb{R} } \\
  & \cup \Set{ (a, \infty) | a \in \mathbb{R} },
\end{aligned}
$$
则 $\mathcal{S}$ 是半代数．
:::
...
\begin{aligned}
  ...
  & \cup \Set{ (a, b] | a < b,\ a, b \in \mathbb{R}^n } \\
...
$$
\begin{aligned}
  & \bigl( (a, b] \setminus \Set{\infty^n} \bigr) \cap \bigl( (c, d] \setminus \Set{\infty^n} \bigr) \\
  ={}& \bigl( (a, b] \cap (c, d] \bigr) \setminus \Set{\infty^n} \\
  ={}& \bigl( a \lor c, b \land d \bigr] \setminus \Set{\infty^n} \in \mathcal{S}.
\end{aligned}
$$
...
$$
\begin{aligned}
  & \bigl( (a, b] \setminus \Set{\infty^n} \bigr]^c = (a, b]^c \\
  ={}& \bigcup_{i = 1}^n \bigl( (a_1, b_1] \times \cdots \times (a_i, b_i]^c \times \cdots \times (a_n, b_n] \bigr),
\end{aligned}
$$
其中只对第 $i$ 分量上的区间取补．由于
$$
  (a_i, b_i]^c = \bigl( (-\infty, a_i] \cup (b_i, \infty) \bigr) \setminus \Set{\infty^n},
$$
我们有
$$
\begin{aligned}
  & (a, b]^c \\
  ={}& \left[ \bigcup_{i = 1}^n \bigl( (a_1, b_1] \times \cdots \times (-\infty, a_i] \times \cdots \times (a_n, b_n] \bigr) \right] \setminus \Set{\infty^n} \\
  \cup{}& \left[ \bigcup_{i = 1}^n \bigl( (a_1, b_1] \times \cdots \times (b_i, \infty) \times \cdots \times (a_n, b_n] \bigr) \right] \setminus \Set{\infty^n} \\
  ={}& \bigcup_{i = 1}^n \bigl( (\tilde{a}_i, \tilde{b}_i] \setminus \Set{\infty^n} \bigr)
  \cup \bigcup_{i = 1}^n \bigl( (\tilde{c}_i, \tilde{d}_i] \setminus \Set{\infty^n} \bigr),
\end{aligned}
$$
其中
$$
...
::: proposition
设 $\Set{ \mathcal{A}_\alpha | \alpha \in I }$ 是一族代数（或 $\sigma$ 代数），则 $\bigcap_{\alpha \in I} \mathcal{A}_\alpha$ 是代数（或 $\sigma$ 代数）．
:::
...
$$
  \mathcal{A}(\mathcal{C}) = \bigcap \Set{ \mathcal{B} | \mathcal{B}\ \text{是}\ \Omega\ \text{上的代数且}\ \mathcal{C} \subseteq \mathcal{B} }
$$
:::
...
$$
  \mathcal{F}(\mathcal{C}) = \bigcap \Set{ \mathcal{B} | \mathcal{B}\ \text{是}\ \Omega\ \text{上的}\ \sigma\ \text{代数且}\ \mathcal{C} \subseteq \mathcal{B} }
$$
:::

由于我们已经有如这个[例子](#ex:semi-algebra-on-R)的半代数，我们希望由它生成一个 $\sigma$ 代数．遗憾的是这样的生成 $\sigma$ 代数的表达非常复杂，但若先放宽要求，则很容易得到生成代数的表达．

::: lemma#lem:generated-algebra-of-semi-algebra
...
$$
  \mathcal{B} := \Set{ \biguplus_{j = 1}^n E_j | n \in \mathbb{N}_{+},\ E_1, \dots, E_n \in \mathcal{S}\ \text{两两不相交} }.
$$
...
::: definition#def:
设 $\mathcal{C} \subseteq \mathscr{P}(\Omega)$ 且 $\emptyset \in \mathcal{C}$，$\mu : \mathcal{C} \to \overline{\mathbb{R}}_{+}$．我们称集合函数 $\mu$ 是**可加的**（additive），若它满足以下性质：
...
我们称集合函数 $\mu$ 是 $\boldsymbol{\sigma}$ **可加的**（$\sigma$-additive），若它满足以下性质：
...
$\sigma$ 可加的集合函数 $\mu : \mathcal{C} \to \overline{\mathbb{R}}_{+}$ 称为 $\mathcal{C}$ 上的**测度**（measure）．

:::

显然 $\sigma$ 可加的集合函数也是可加的．
...
对任意集合 $A \subseteq \Omega$，我们定义**指示函数**（incidator function）$1_A : \Omega \to \Set{0, 1}$ 为
$$
  1_A(x) := \begin{cases}
    0, & x \notin A, \\
    1, & x \in A.
...
::: example 离散测度
设 $\Omega = \Set{ x_1, x_2, \dots }$ 是可数集，$p_1, p_2, \dots \in \mathbb{R}_{+}$．定义集合函数
$$
\begin{aligned}
  \mu : \mathscr{P}(\Omega) &\to \overline{\mathbb{R}}_{+}, \\
  A &\mapsto \sum_{j \geq 1} p_j 1_A(x_j),
\end{aligned}
$$
则 $\mu$ 是 $\sigma$ 可加的．
:::
...
::: proof
显然 $\mu(\emptyset) = 0$．任取两两不相交的 $A_1, A_2, \dots \subseteq \Omega$，令 $A := \biguplus_{j \geq 1} A_j$．对任意 $x \in \Omega$，很容易验证 $1_A(x) = 1$ 当且仅当 $\sum_{k \geq 1} 1_{A_k}(x) = 1$，也就是
$$
  1_A = \sum_{k \geq 1} 1_{A_k},
$$
因此
$$
\begin{aligned}
  \mu(A) &= \sum_{j \geq 1} p_j 1_{A}(x_j) \\
  &= \sum_{j \geq 1} \sum_{k \geq 1} p_j 1_{A_k}(x_j) \\
  &= \sum_{k \geq 1} \sum_{j \geq 1} p_j 1_{A_k}(x_j) \\
  &= \sum_{k \geq 1} \mu(A_k),
\end{aligned}
$$
即 $\mu$ 是 $\sigma$ 可加的．
...
::: example#ex:additive-but-not-sigma-additive
设 $\Omega = (0, 1)$，定义集合系
$$
  \mathcal{C} := \Set{ (a, b] | 0 \leq a < b \leq 1 }
$$
以及集合函数
$$
\begin{aligned}
  \mu : \mathcal{C} &\to \overline{\mathbb{R}}_{+}, \\
...
\end{aligned}
$$
则 $\mu$ 是可加的，但不是 $\sigma$ 可加的．
:::

::: proof
先证明 $\mu$ 不是 $\sigma$ 可加的．考虑分解
$$
  \left( 0, \frac{1}{2} \right] = \biguplus_{j \geq 1} \left(\frac{1}{2 + j}, \frac{1}{1 + j} \right].
$$
$\mu$ 作用在左侧得到 $\infty$，作用在右侧得到
$$
  \sum_{j \geq 1} \left( \frac{1}{1 + j} - \frac{1}{2 + j} \right) = \frac{1}{2},
$$
两者不相等．

下面证明 $\mu$ 是可加的．设 $(a, b] \in \mathcal{C}$ 能写成 $\mathcal{C}$ 中两个不相交元素 $(a_1, b_1]$ 和 $(a_2, b_2]$ 的并．若 $a_1$ 和 $a_2$ 中有任意一者为 $0$，则 $a = 0$，因此
$$
\begin{aligned}
  \mu\bigl( (a, b] \bigr) &= \infty \\
  &= \mu\bigl( (a_1, b_1] \bigr) + \mu\bigl( (a_2, b_2] \bigr).
\end{aligned}
$$
若 $a_1$ 和 $a_2$ 均不为零，不妨设 $a_1 \leq a_2$，则必然有 $b_1 = a_2$、$a_1 = a$ 和 $b_2 = b$，因此
$$
\begin{aligned}
  & \mu\bigl( (a_1, b_1] \bigr) + \mu\bigl( (a_2, b_2] \bigr) \\
  ={} & (b_1  - a_1) + (b_2 - a_2) \\
