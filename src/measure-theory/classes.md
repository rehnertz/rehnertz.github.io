...
\end{aligned}
$$
在下面的论述中，我们将 $(a, b] \cap \mathbb{R}^n$ 简写为 $(a, b]^\circ$，则 $A$ 的补集 $\mathbb{R}^n \setminus A$ 可以写成
$$
\begin{aligned}
  & \mathbb{R}^n \setminus A \\
  ={}& \mathbb{R}^n \setminus (a, b]^\circ \\
  ={}& \bigcup_{i = 1}^n \Bigl( (a_1, b_1]^\circ \times \cdots \times (a_{i - 1}, b_{i - 1}]^\circ \\
  & \times \bigl( (-\infty, a_i]^\circ \cup (b_i, \infty]^\circ \bigr) \\
  & \times (a_{i + 1}, b_{i + 1}]^\circ \times \cdots \times (a_n, b_n]^\circ \Bigr) \\
  ={}& \bigcup_{i = 1}^n \bigl( (\alpha^i, \beta^i]^\circ \cup (\gamma^i, \delta^i]^\circ \bigr),
\end{aligned}
$$
